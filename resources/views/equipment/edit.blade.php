...
@section('content')
    ...
    <div class="card" style="max-width: 800px;">
        <form id="equipment-edit-form" action="{{ route('equipment.update', $equipment) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="partner_id" class="form-control" required>
                        @foreach($partners as $partner)
                            <option value="{{ $partner->id }}" {{ old('partner_id', $equipment->partner_id) == $partner->id ? 'selected' : '' }}>
                                {{ $partner->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <input type="date" name="received_at" class="form-control"
                    value="{{ old('received_at', optional($equipment->received_at)->format('Y-m-d')) }}">
            </div>
    ...
    <script>
        ...
        const storageKey = 'equipment_edit_draft';
